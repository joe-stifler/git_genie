...
class MarkdownGenerator:
    ...
    def generate_pr_description(commit_history, template_path):
        ...
        try:
            ...
            logging.warning(
                "Failed to read the Pull Request template file at: %s", template_path
            )
            sys.exit(1)
        ...
        markdown_content += "3. Check which files are created, which ones are removed, and which ones are updated.\n"
        markdown_content += "4. Then think step by step and ensure that your generated the correct PR description by checking the patch json content generated from git diff provided above.\n"
        markdown_content += "5. Finally, create a pull request title that follows the semantic versioning standards.\n"
